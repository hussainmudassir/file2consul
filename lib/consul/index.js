...
var diff_kvs = function(write_kvs, delete_kvs, candidate_kvs, existing_kvs) {
  ...
  candidate_kvs.forEach(function(kv) {
    ...
    if (idx === -1) {
      ...
      write_kvs.push(kv);
    } else if (kv.value === null || kv.value.toString() !== existing_kvs[idx].value) {
...
var file_modified = function(config_file, cb) {
  var handle_json_kv_file = function(kvs, cb) {
    fs.readFile(config_file, {encoding: 'utf8'}, function (err, body) {
      if (err) return cb('Failed to read key ' + config_file + ' due to ' + err);
      body = body ? body.trim() : '';
      write_content_to_consul(config_file, body, cb);
    });
  };
  ...
  var handle_properties_kv_file = function(kvs, cb) {
    function extract_and_populate_properties(file_body, common_body, cb) {
      ...
      populate_kvs_object(obj, config_file, kvs, cb);
      });
    };

    fs.readFile(config_file, {encoding: 'utf8'}, function (err, file_body) {
      if (err) return cb('Failed to read key ' + config_file + ' due to ' + err);
  ...
  get_kvs(config_file, function(err, kvs) {
    if(err) cb("Failed to get kvs of existing");
    handle_expanded_keys_with_different_file_types(kvs);
  });
};

var get_kvs = function(config_file, cb) {
  // Prepend branch name to the KV so that the subtree is properly namespaced in Consuls
  console.log('Getting tree for key %s', config_file);

  consul.kv.get({'key': config_file + '/', token: token, recurse: true}, function(err, kvs, res) {
    if (err) return cb('Failed to get tree for key ' + config_file + ' due to ' + err, undefined);
...
exports.updateConfig = function(config_file, cb) {
  file_modified(config_file, function(err) {
    if(err) {
      console.log("dekhna padega", err);
      cb(err);
    } else {
      clog.table(result);
      cb();
